...
def main():
    ...
    # 2. ENTRADA DE CONFIGURAÇÃO DA CAMPANHA (MANTIDO SEU MODELO ORIGINAL DO PRINT)
    ...
    if botao_gerar and produto_nome:
        ...
        # =============================================================================================================
        # 3. CENTRAL DE PALAVRAS-CHAVE EXCLUSIVAS (4 COLUNAS HORIZONTAIS COMPACTADAS INLINE DE SUCESSO)
        # =============================================================================================================
        ...
        # Distribuição horizontal plana achatada imune a desalinhamentos de recuo
        c_solta, c_aspas, c_colchete, c_negativa = st.columns(4)

        s_broad = ["official store", "buy online", "best price", "where to buy", "purchase original", "order discount", "secure package", "promo code", "retailer store", "sale online", "safest site", "lowest cost", "supply near me", "get bottles", "shop discount"]
        s_phrase = ["official website", "supplement reviews", "ingredients list", "customer warning", "independent review", "real side effects", "fda approved status", "capsules directions", "weight loss drops", "complaints check", "scam alert report", "shipping tracking", "refund policy guarantee", "clinical studies results", "formula benefits"]
        s_exact = ["brand bidding", "manufacturer direct", "authorized seller", "coupon code 2026", "moneyback guarantee", "exclusive offer matinal", "certified pure check", "stock availability", "wholesale price package", "official link gate", "verified checkout page", "vip client portal", "one time payment", "secured order processing", "original product checkout"]
        kw_neg = ["free", "scam", "fake", "complaints", "side effects", "amazon", "ebay", "walmart", "bad review", "alternative", "ingredients", "cancer", "diabetes", "medical doctor", "hoax"]

        with c_solta:
            st.markdown("<h4>🟢 15 Amplas (Broad)</h4>", unsafe_allow_html=True)
            st.text_area("Copiar Soltas:", value="\n".join([f"{p_nome} {s}" for s in s_broad]), height=320, key="kw_soltas")

        with c_aspas:
            st.markdown("<h4>🔵 15 Frases (Phrase)</h4>", unsafe_allow_html=True)
            st.text_area("Copiar Frases:", value="\n".join([f'"{p_nome} {s}"' for s in s_phrase]), height=320, key="kw_aspas")

        with c_colchete:
            st.markdown("<h4>🔴 15 Exatas (Exact)</h4>", unsafe_allow_html=True)
            st.text_area("Copiar Exatas:", value="\n".join([f"[{p_nome} {s}]" for s in s_exact]), height=320, key="kw_colchetes")

        with c_negativa:
            st.markdown("<h4>❌ 15 Negativas (Negative)</h4>", unsafe_allow_html=True)
            st.text_area("Copiar Negativas:", value="\n".join(kw_neg):, height=320, key="kw_negativas")
